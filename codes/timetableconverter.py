...
class TimeTable:
    ...
    def readfile(self, png):
        ...
        img_bin = cv2.adaptiveThreshold(
            self.img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2
        )
        # inverting the image
        img_bin = 255 - img_bin
            ...
            for j in range(len(finalboxes[i])):
                ...
                if len(finalboxes[i][j]) == 0:
                    ...
                    for k in range(len(finalboxes[i][j])):
                        ...
                        inner = inner + " " + out
                    outer.append(inner)
                self.progress = i / len(finalboxes) + 1 / len(finalboxes) * (
                    (j + 1) / len(finalboxes[i])
                )
        ...
        if self.debug:
            dataframe.to_pickle(
                os.path.join(os.getcwd(), "Data/{}.pkl".format(self.id))
            )
